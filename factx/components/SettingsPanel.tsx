...
export default function SettingsPanel({ onClose }: SettingsPanelProps) {
  const [isEnabled, setIsEnabled] = useStorage<boolean>("factx-enabled", true)
  const [semanticFilter, setSemanticFilter] = useStorage<string>("factx-semantic-filter", "")
  const [excludedKeywords, setExcludedKeywords] = useStorage<string>("factx-excluded-keywords", "")
      ...
      >
        <header className="p-4 border-b border-gray-800">
          <div className="flex items-center gap-3">
            ...
            />
            <div>
              <h2 className="text-xl font-semibold text-white">FactX Settings</h2>
              <p className="text-gray-400 text-sm mt-1">
                Configure how FactX analyzes your feed
              </p>
            </div>
          </div>
        </header>
